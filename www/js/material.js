"use strict";
jQuery.noConflict(); // 释放 jQuery 对 $ 变量的控制
/* Util */
...
var Dialog = {
	...
	toggle: function(dialog) {
		var b = !dialog.hidden,
			ho = isNull(this.overlay);
		if(b){
			if(!ho)	this.overlay.hidden = b;
		}
		else {
				if(ho)this.createOverlay();
				this.overlay.hidden = b;
		}
		dialog.hidden = b;
	},
